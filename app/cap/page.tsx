...
export default function CapPage(){
  ...
  const [initialCapLoaded, setInitialCapLoaded] = useState(false);

  const saveBtnRef = useRef<HTMLButtonElement>(null);

  // ========= โหลด time-window + capRule ล่าสุด =========
  useEffect(()=>{(async()=>{
    ...
    // --- ดึง CapRule แถวล่าสุดมาเติม state ---
    try {
      const capRes = await fetch('/api/cap/preview',{method:'GET', cache:'no-store'});
      if (capRes.ok) {
        const cap = await capRes.json() as CapLatestResp;
        if (cap && cap.hasCap) {
      ...
      console.error('load latest cap failed', e);
    } finally {
      setInitialCapLoaded(true);
    }
  })()},[]);

  // เปลี่ยนงวด → sync datetime-local
  useEffect(()=>{
  ...
  }, [fromUTC, toUTC, autoPreviewDone, initialCapLoaded]);

  async function callCapApi(action:'preview'|'preview_and_save'){
    const res = await fetch('/api/cap/preview', {
      method: 'POST',
      ...
      <div className="border rounded p-3 space-y-3">
        <div className="flex items-center gap-4">
          ...
          <label className="inline-flex items-center gap-2 ml-6">
            ...
            />
            <span>แปลง 3 โต๊ด → 3 ตัวบน (ใช้กับ Auto/Manual)</span>
          </label>
        </div>
