    ...
    <div className="max-w-[1100px] mx-auto px-3 py-4 space-y-3">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border">
          ...
          <tbody>
            {/* แปลง rows -> กระจายตามคอลัมน์หมวด */}
            {rows.length === 0 ? (
              <tr>
              ...
              // กลุ่มทีละบรรทัด: render เป็นบล็อกแยกตามหมวดด้วย key = category
              // ฝั่ง API /settle-view ส่ง items เป็น (category, number, totalSend)
              rows.map((r, idx) => (
                <tr key={`${r.category}-${r.number}-${idx}`}>
                  {[
                    'TOP3',
                    'TOD3',
                    'TOP2',
                    ...
                    <td key={cat} className="border px-2 py-1 align-top">
                      {r.category === cat ? `${r.number} = ${Number(r.totalSend)}` : ''}
                    </td>
                  ))}
                </tr>
              ))
